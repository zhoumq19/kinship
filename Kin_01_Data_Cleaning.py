...
def input_data_preprocessing(data_dir, data='CBDB_20201110.db'):
    ...
    with closing(sqlite3.connect(CBDB_file)) as conn:
        with conn:  # auto-commits
            ...
            ALTNAME_DATA = pd.read_sql("SELECT * FROM ALTNAME_DATA", conn, coerce_float=False)
            DYNASTIES = pd.read_sql("SELECT * FROM DYNASTIES", conn, coerce_float=False)
            # ADDRESSES = pd.read_sql("SELECT * FROM ADDRESSES", conn)
            # ADDR_CODES = pd.read_sql("SELECT * FROM ADDR_CODES", conn)

            # OFFICE_CODES = pd.read_sql("SELECT * FROM OFFICE_CODES", conn)
